...
// jamie hacks
var TB_VERTICAL_SHIFT = 0;  // was 70
...
//					console.log("TB_Counter: "+TB_Counter+"  TB_TempArray.length:"+TB_TempArray.length+" url:"+url+"  TB_TempArray[TB_Counter].href"+TB_TempArray[TB_Counter].href);
					if (!(TB_TempArray[TB_Counter].href == url)) {						
						...
						TB_CounterFake = false; //jamiehack
					} else {
						TB_FoundURL = true;
...
*/    			
			...
			TB_position();
			$("#TB_load").remove();
            // ### jamie hack ### click = next if another one, close otherwise
            // also, if there's only one just close it (no real math required)
...
function TB_position() {
	...
	$("#TB_window").css( {  width:TB_WIDTH+"px", 
	                        ...
	                        top: (arrayPageScroll[1] + (pagesize[1]-TB_HEIGHT)/2 - TB_VERTICAL_SHIFT)+"px" 
	                    } );
}
